...
describe("VehicleNFT Purchase", function () {
  ...
  it("Should allow certified user to purchase vehicle", async function () {
    // await expect(vehicleNFT.connect(buyer).purchaseVehicle(newItemId, { value: price }))
    //   .to.emit(vehicleNFT, "OwnershipTransferred")
    //   .withArgs(newItemId, manufacturer.address, buyer.address, price);
      
      await expect(vehicleNFT.connect(buyer).purchaseVehicle(tokenId, {
        value: ethers.utils.parseEther("1")
      }))
        .to.emit(vehicleNFT, "OwnershipTransferred")
        .withArgs(tokenId, owner.address, buyer.address, ethers.utils.parseEther("1"));
  ...
  it("Should not allow purchase with incorrect payment amount", async function () {
    ...
    await expect(vehicleNFT.connect(buyer).purchaseVehicle(newItemId, { value: incorrectPrice }))
  ...
  it("Should not allow uncertified user to purchase vehicle", async function () {
    ...
    await expect(vehicleNFT.connect(uncertifiedBuyer).purchaseVehicle(newItemId, { value: price }))
